...
<template>
  ...
  >
    <t-date-picker v-model="value" v-bind="$attrs" />
  </t-form-item>
  <t-form-item
    :label="config?.label"
    :labelWidth="config?.labelWidth"
...
const options = computed(() => {
  const select = props.config.attrs.options.find(item => item.value === props.typeValue);
  return select?.children;
});

const attrOptions = computed(() => {
  const options = props.config.attrs.options;
  return options ? options.map(item => { return { value: item.value, label: item.label }}) : [];
});
