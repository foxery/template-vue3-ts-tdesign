...
const transform: AxiosTransform = {
  ...
  transformRequestHook: (res, options) => {
    ...
    if (
      res.headers['content-type']?.indexOf('application/vnd.openxmlformats-officedocument.spreadsheetml.sheet') > -1
    ) {
      // excel二进制文件流
      let str = res.headers['content-disposition'] || 'file.xlsx';
  ...
  requestInterceptors: (config, options) => {
    ...
    if (userInfo.token && (config as Recordable)?.requestOptions?.withToken !== false) {
      // jwt token
      (config as Recordable).headers.Authorization = options.authenticationScheme
        ? `${options.authenticationScheme} ${userInfo.token}`
        : userInfo.token;
    }
    console.log(`output->config+userInfo`, userInfo, config);
    return config;
  },
