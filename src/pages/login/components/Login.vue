...
const onSubmit = async ({ validateResult }) => {
  if (validateResult === true) {
    try {
      ...
      const { token, user } = await Login(account, MD5(password).toString());
      await userStore.setToken(token);
      await userStore.setUserInfo({ nickname: user.account, roles: user.routes });
      MessagePlugin.success('登录成功');
      const redirect = route.query.redirect as string;
      const redirectUrl = redirect ? decodeURIComponent(redirect) : '/welcome/index';
